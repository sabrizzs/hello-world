...
#include <iostream>
#include <stack>
#include <string> //strlen (dps substituir)
    ...
    for(int i = 0; i<strlen(pal); i++){
        ...
        for(int j = 0; j<G.size(); j++){
            ...
            //barra
                ...
                cout<< "achei o barra barra no i: " << i << endl;
                if(letra[j+1] == pal[i]){
                    prox[j+2] = true;
                    cout << "barra barra eh igual " << letra[j] << " da palavra " << pal[i] << endl; 
                }       
            //coringa          
            } else if(atingidos[j] && letra[j] == '.'){
                cout << "eh ponto: " << i << " da palavra ( " << pal[i] << " ) tem caminho para " << j << " da regex: " << letra[j] << endl;
                prox[j+1] = true;
            //colchetes
            } else if(atingidos[j] && letra[j] == '['){
                int a = j;
                ...
                //intervalo
                    ...
                    if(isdigit(aux)){
                        ...
                        for(int b = 0; b<=26; b++){
                            if(alfabeto[b] == letra[a+1]) inicio = b;
                            if(alfabeto[b] == letra[a+3]) fim = b;
                        }
                        for(int b = inicio; b<=fim; b++){
                            if(alfabeto[b] == pal[i]) prox[j+1] = true;
                        }
                    }
