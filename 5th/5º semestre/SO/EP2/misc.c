 ...
 *				FUNÇÃO DO EP			       	     *
 *===========================================================================*/
//precisa declarar a função em algum lugar

PUBLIC int do_lockpriority(int pid, int priority) {
    
    /* Verifica se o pid é um processo filho */
    if (is_child(proc_ptr, pid) != OK) {
        return -2; /* Retorna -2 se o pid não é um processo filho */
    }

    /* Verifica se a priority é uma prioridade válida */
    if (priority < USER_Q || priority > MAX_USER_Q) {
        return -1; /* Retorna -1 se a priority é inválida */
    }

    /* Muda a prioridade do processo filho */
    proc_t* child_proc_ptr = proc_addr(pid);
    child_proc_ptr->p_priority = priority;

    return 0; /* Retorna 0 se a mudança de prioridade foi bem sucedida */
}
