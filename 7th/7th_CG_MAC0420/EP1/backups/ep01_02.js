...
function main(){
    ...
    pauseButton.addEventListener('click', pauseCallback(pauseButton));
    ...
    if(isRunning){
        if(isTimerMode){
            if(currentTime < typedTime){
                updateDisplay();
                requestAnimationFrame(() => timeUpdate(startTime));
            } else {
                resetCallback()
            }       
        } else {
...
function pauseCallback(pauseButton){
    return function(){
