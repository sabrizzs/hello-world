...
class Inseticida {
    constructor() {
        this.larguraBase = (LAR_INSETICIDA * w)/2;
    ...
    giraBicoAntihorario() {
        if (this.anguloBico > ANG_MIN_INSETICIDA) {
            ...
            gObjetos[2].rotaciona(ANG_PASSO);
...
class Tiro {
    ...
    desenha() {
        ...
        gObjetos.push(new Disco(this.xBico, this.yBico, 10, this.vx, this.vy, sorteieCorRGBA()));
    }
...
class Mosquito {
    constructor(index) {
        ...
        let limiteX = gCanvas.width - 100 - gCanvas.width * BORDA;
        let limiteY = gCanvas.height - 100 - gCanvas.height * BORDA;
        let inicioX = gCanvas.width * BORDA;
        let inicioY = gCanvas.height * BORDA;

        this.x = Math.random() * limiteX + inicioX;
        this.y = Math.random() * limiteY + inicioY;
...
function Triangulo(x, y, lado, vx, vy, cor, orientacao = 1, mosquito) {
    ...
    this.atualize = function (delta, x, y) {
        ...
        if(mosquito){
            ...
            var limAsaAberta = 20 * Math.PI / 180;
            var limAsaFechada = -20 * Math.PI / 180;

            // Bate as asas
            if (this.asaAberta) {
                // Posição das asas abertas
                this.angAsa += orientacao * 10 * Math.PI / 180;
                let cosAngulo = Math.cos(this.angAsa);
                let sinAngulo = Math.sin(this.angAsa);

                for (let i = 0; i < this.nv; i++) {
                    // Aplica a rotação a cada vértice
                    let vertice = this.vertices[i];
                    let novoX = vertice[0] * cosAngulo - vertice[1] * sinAngulo;
                    let novoY = vertice[0] * sinAngulo + vertice[1] * cosAngulo;
                    this.vertices[i] = vec2(novoX, novoY);
                }
            } else {
            ...
            if(this.angAsa >= limAsaAberta) this.asaAberta = !this.asaAberta;
            if(this.angAsa <= limAsaFechada) this.asaAberta = !this.asaAberta;

            let corpoDengue = (LAR_DENGUE / 2) * h;

            // Colisão com as bordas
            const bordaW = gCanvas.width - gCanvas.width * BORDA;
            const bordaH = gCanvas.height - gCanvas.height * BORDA;
            
            if(orientacao === 1){
                if (x < gCanvas.width * BORDA + corpoDengue) { vx = -vx; };
                if (x  >= bordaW + corpoDengue) { vx = -vx; };
            } else {
                if (x < gCanvas.width * BORDA - corpoDengue) { vx = -vx; };
                if (x + corpoDengue >= bordaW ) { vx = -vx; };
            }
            
            if (y < gCanvas.height * BORDA) { vy = -vy; };
            if (y >= bordaH) { vy = -vy; };
    ...
    this.rotaciona = function(angulo) {
        let anguloRad = angulo * Math.PI / 180;

        // Calcula o cosseno e o seno do ângulo
        let cosAngulo = Math.cos(anguloRad);
        let sinAngulo = Math.sin(anguloRad);

        // Percorre todos os vértices do triângulo
        for (let i = 0; i < this.nv; i++) {
            // Aplica a rotação a cada vértice
            let vertice = this.vertices[i];
            let novoX = vertice[0] * cosAngulo - vertice[1] * sinAngulo;
...
function Quadrado(x, y, lado, vx, vy, cor, mosquito, index) {
    ...
    this.atualize = function (delta, x, y) {
        ...
        if(mosquito){
            ...
            const bordaW = gCanvas.width - gCanvas.width * BORDA;
            const bordaH = gCanvas.height - gCanvas.height * BORDA;
            if (x < gCanvas.width * BORDA) { vx = -vx; };
            if (y < gCanvas.height * BORDA) {  vy = -vy; };
            if (x >= bordaW) { vx = -vx; };
            if (y >= bordaH) { vy = -vy; };
