    ...
    verificaColisao() {
        for (let i = 0; i < mosquitos.length; i++) {
            ...
            if (this.x >= mosquito.x - LAR_DENGUE * h && this.x <= mosquito.x + LAR_DENGUE * h &&
                ...
                // Colisão detectada
                console.log(gObjetos);
                console.log('Colisão com mosquito!');  
                console.log(i+1);
...
function criaMosquitos(numMosquitos) {
    for (let i = 0; i < numMosquitos; i++) {
        mosquitos.push(new Mosquito(i+1));
    }
}
...
function Quadrado(x, y, lado, vx, vy, cor, mosquito, index) {
    ...
    this.atualize = function (delta, x, y) {
        ...
        if(mosquito){
            ...
            if (index !== undefined && index > 0 && mosquitos[index - 1] !== undefined) {
                mosquitos[index - 1].x = this.pos[0];
                mosquitos[index - 1].y = this.pos[1];
            }
